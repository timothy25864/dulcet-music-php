...
if( $_FILES["PVideo"]["error"] === 0 ) {
    ...
    if( move_uploaded_file($_FILES["PVideo"]["tmp_name"], "./files/".$PVideo) ) {
        ...
        //先查詢出特定 PId (editId) 資料欄位中的團片檔案名稱
        $sqlGetVideo = "SELECT `PVideo` FROM `product` WHERE `PId` = ? ";
        $stmtGetVideo = $pdo->prepare($sqlGetVideo);
        ...
        //執行 SQL 語法
        $stmtGetVideo->execute($arrGetVideoParam);

        //若有找到 PImg 的資料
        if($stmtGetVideo->rowCount() > 0) {
            //取得指定 PId 的資料 (1筆)
            $arrVideo = $stmtGetVideo->fetchAll(PDO::FETCH_ASSOC);

            //若是 PImg 裡面不為空值，代表過去有上傳過
            if($arrVideo[0]['PVideo'] !== NULL){
                //刪除實體檔案
                @unlink("./files/".$arrVideo[0]['PVideo']);
            } 
